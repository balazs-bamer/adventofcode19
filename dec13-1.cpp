...
void play(std::ifstream &aIn) {
  int constexpr cEmpty  = 0;
  int constexpr cWall   = 1;
  int constexpr cBlock  = 2;
  int constexpr cPaddle = 3;
  int constexpr cBall   = 4;

  std::map<std::pair<int, int>, int> screen;
  Intcode computer(aIn);
  computer.start();
  std::cout << "halted: " << computer.run() << '\n';
  while(computer.hasOutput()) {
    std::pair<int, int> coordinates{computer.output().toInt(), computer.output().toInt()};
    int newElement = computer.output().toInt();
    auto found = screen.find(coordinates);
    if(newElement == cEmpty) {
      if(found != screen.end()) {
        screen.erase(found);
      }
      else { // nothing to do
      ...
      screen[coordinates] = newElement;
    }
  }
  size_t count = std::count_if(screen.begin(), screen.end(), [](auto &i){
    return i.second == cBlock;
  });
  std::cout << count << '\n';
}
