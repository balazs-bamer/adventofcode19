...
public:
  ...
  Coordinates(int const aX, int const aY) : x(aX), y(aY) {
  }
...
public:
  Labyrinth(std::ifstream &aIn) : mComputer(aIn), mLocation(0, 0) {
    mComputer.start();
  }

  size_t getShortestPathLengthToOxygen() noexcept {
    ...
    while(true) {
      ...
      if(newDirection == Node::cNoParent) {
        ...
        if(moveRobot(newDirection) == cOxygen) {
          shortest = (mLocation.isOrigin() ? 0 : mIteration);
          break;
        }
        else { // nothing to do
        }
...
int main(int const argc, char **argv) {
  ...
  try {
    ...
    size_t pathLength = labyrinth.getShortestPathLengthToOxygen();
