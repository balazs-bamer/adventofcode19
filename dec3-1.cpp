  ...
  int getNearestIntersectionDistance(Collector &aOther) const {
    ...
    if(mHorizontalStart) {
      if(horizontal != mHorizontals.end()) {
        distance = std::min(distance, aOther.measure(x, y, *horizontal, true));
        x += *horizontal;
        ++horizontal;
      }
      else { // nothing to do
    ...
    for(;;) {
      if(vertical != mVerticals.end()) {
        distance = std::min(distance, aOther.measure(x, y, *vertical, false));
        y += *vertical;
        ++vertical;
      }
      else {
...
private:
  int measure(int aStartX, int aStartY, int aDisplacement, bool const aHorizontal) const {
    int distance = std::numeric_limits<int>::max();
    int x = 0;
    int y = 0;
    auto horizontal = mHorizontals.begin();  
    ...
    if(mHorizontalStart) {
      if(horizontal != mHorizontals.end()) {
        if(!aHorizontal) {
          distance = std::min(distance, testIntersection(x, y, *horizontal, aStartX, aStartY, aDisplacement));
        }
        else { // nothing to do
        }
        x += *horizontal;
    ...
    for(;;) {
      if(vertical != mVerticals.end()) {
        if(aHorizontal) {
          distance = std::min(distance, testIntersection(aStartX, aStartY, aDisplacement, x, y, *vertical));
        }
        else {
        }
        y += *vertical;
  ...
  int testIntersection(int const aHorStartX, int const aHorStartY, int const aHorDisplacement, int const aVertStartX, int const aVertStartY, int const aVertDisplacement) const noexcept {
    int distance = std::numeric_limits<int>::max();
    if(contains(aVertStartY, aVertDisplacement, aHorStartY) && contains(aHorStartX, aHorDisplacement, aVertStartX)) {
      distance = abs(aHorStartY) + abs(aVertStartX);
    }
    else { // nothing to do
    }
    return distance;
